...
module Decidim
  module Generators
    ...
    class InstallGenerator < Rails::Generators::Base
      ...
      def rails(*args)
        with_original_env do |env|
          abort unless system(env, "bin/rails", *args)
        end
      end
      ...
      def soft_rails(*args)
        with_original_env do |env|
          system(env, "bin/rails", *args, err: File::NULL)
        end
      end

      def with_original_env
        options[:skip_gemfile] ? yield({}) : Bundler.with_original_env { yield("BUNDLE_GEMFILE" => "Gemfile") }
      end
